...
{
    internal class MotoreOrarioTrainScheduleExpertPlugin
    {
        ...
        public async Task<List<TrainModel>> GetTrainsByDateCityStartCityArrival(DateTime date, string cityStart, string cityArrival)
        {
            ...
            var trains = this._trainModels.Where(train => (train.OrarioPartenza.Date.Year == date.Date.Year && train.OrarioPartenza.Date.Month == date.Date.Month && train.OrarioPartenza.Day == date.Date.Day)
